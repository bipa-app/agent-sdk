...
/// Checkpoint store that reports `None` for the first N `get_by_turn`
/// reads even when the row exists — the in-memory committer's
/// aggregate-then-checkpoint gap (codex round-2 P2).
struct GappyCheckpointStore {
    inner: InMemoryCheckpointStore,
    misses: AtomicUsize,
}

#[async_trait]
impl CheckpointStore for GappyCheckpointStore {
    async fn commit_checkpoint(
        &self,
        params: crate::journal::checkpoint::NewCheckpointParams,
    ) -> Result<crate::journal::checkpoint::Checkpoint> {
        self.inner.commit_checkpoint(params).await
    }
    async fn get(
        &self,
        id: &crate::journal::checkpoint::CheckpointId,
    ) -> Result<Option<crate::journal::checkpoint::Checkpoint>> {
        self.inner.get(id).await
    }
    async fn get_by_turn(
        ...
        self.inner.get_by_turn(thread_id, turn_number).await
    }
    async fn get_latest_by_thread(
        &self,
        thread_id: &ThreadId,
    ) -> Result<Option<crate::journal::checkpoint::Checkpoint>> {
        self.inner.get_latest_by_thread(thread_id).await
    }
    async fn list_by_thread(
        &self,
        thread_id: &ThreadId,
    ) -> Result<Vec<crate::journal::checkpoint::Checkpoint>> {
    ...
    async fn threads_exceeding_checkpoint_count(
        ...
        self.inner
            .threads_exceeding_checkpoint_count(limit, batch)
            .await
    }
    async fn delete_checkpoints_beyond_limit(
        ...
        self.inner
            .delete_checkpoints_beyond_limit(thread_id, limit)
            .await
    }
}

/// Codex round-2 P2: the occupancy lookup retries through the in-memory
/// committer's aggregate-then-checkpoint gap — a transiently invisible
/// salvage checkpoint must not turn a shiftable collision terminal.
#[tokio::test]
async fn shift_retries_through_the_checkpoint_visibility_gap() -> Result<()> {
    let stores = TestStores::new();
    stores.threads.get_or_create(&thread_a(), t0()).await?;
    ...
    let salvager_id = AgentTaskId::from_string("task_gap_salvager");
    let racing = SalvageRacingAttemptStore {
        inner: stores.attempts.clone(),
        stores: stores.clone(),
        salvager_task: salvager_id.clone(),
    ...
    let gappy = GappyCheckpointStore {
        inner: stores.checkpoints.clone(),
        misses: AtomicUsize::new(2),
    };
    let mut deps = stores.deps();
    deps.attempt_store = &racing;
    deps.checkpoint_store = &gappy;

    let provider = MockTextProvider::new("successor answer");
    let outcome = execute_root_turn(inputs, "run the successor", &provider, &deps, t_plus(5))
        .await
        .context("the shift must survive the checkpoint visibility gap")?;
    let RootTurnOutcome::Completed { completed_task, .. } = outcome else {
        bail!("expected Completed, got a suspension");
    };
    assert_eq!(completed_task.status, TaskStatus::Completed);
