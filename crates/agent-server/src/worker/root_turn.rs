...
use super::definition::{
    AgentDefinition, ChildJoinPolicy, ThinkingPolicy, ToolInputPreflightContext,
...
use crate::journal::store::{AgentTaskStore, QuestionPause, ReattachedChild};
...
struct StreamedTurn {
    /// `response.usage` carries the *turn* total: the successful attempt's
    /// tokens plus those billed by earlier recoverably-failed attempts. Every
    /// turn-level and cumulative usage sink derives from it via
    /// [`response_token_usage`], so folding here bills the whole turn once.
    response: llm::ChatResponse,
    content_ids: ContentIds,
    attempt: TurnAttempt,
    /// The successful attempt's *own* usage, for its audit row. Kept separate
...
pub async fn revert_steering_wake(
    ...
    let payload = SuspensionPayload {
        ...
        child_join_policy: parent.state.child_join_policy(),
    };
...
/// 2026-07-16 incident held Anthropic 529s for 30+ minutes and the
/// previous shared 3-retry/<5 s budget terminally failed every
/// in-flight turn, including multi-tool turns whose work was already
/// durably committed and only needed one more model round to deliver.
/// Ten retries on the [`RATE_LIMIT_BASE_DELAY_MS`] schedule give ~8.5
...
/// A provider-supplied `Retry-After` hint still wins over this
/// schedule (clamped by [`STREAM_MAX_RETRY_AFTER_MS`]).
...
enum StreamAttemptError {
    ...
    Recoverable {
        ...
        message: String,
        /// Usage the failed attempt had already streamed. The provider
        /// billed those tokens whether or not the turn survived, so they
        /// travel to the retry wrapper and are folded into the eventual
        /// successful response's turn usage — the same seam the in-process
        /// loop uses. The failed attempt's own audit row bills this
        /// separately (see [`close_attempt_with`]); the two ledgers do not
        /// overlap, so there is no double-count.
        usage: llm::Usage,
    },
...
impl StreamAttemptError {
    /// Record the usage the failed attempt had streamed.
    ///
    /// Only a recoverable error is retried, so only it can strand usage a later
    /// attempt must carry; the terminal variants end the turn and are returned
    /// unchanged.
    fn with_usage(self, usage: llm::Usage) -> Self {
        match self {
            Self::Recoverable { kind, message, .. } => Self::Recoverable {
                kind,
                message,
                usage,
            },
...
/// Assemble the [`StreamedTurn`] from a successful attempt, splitting usage
/// between the attempt's own audit row and the whole-turn total.
///
/// The successful attempt's own usage bills its audit row; the turn total
/// (`response.usage`, read by every turn/cumulative sink) additionally absorbs
/// the tokens billed by earlier recoverably-failed attempts. Keeping the two
/// apart bills each attempt's row once and the turn once, with no cross-row
/// double-count.
...
struct WorkerRetryState {
    /// Bounded server-error-class transient failures, capped at
    /// [`STREAM_MAX_RETRIES`].
    transient: u32,
    /// Bounded rate-limited failures, capped at the deliberately
    /// deeper [`RATE_LIMIT_MAX_RETRIES`]. Kept separate from
    /// `transient` so an overload window and a server-error streak
    /// spend independent budgets.
    rate_limit: u32,
...
impl WorkerRetryState {
    ...
    const fn any_stream_retry(&self) -> bool {
        self.transient > 0 || self.rate_limit > 0 || self.connectivity > 0
    }
}
...
async fn call_llm_with_retry(params: LlmRetryParams<'_>) -> Result<StreamedTurn> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(StreamAttemptError::Recoverable {
                kind,
                message,
                usage,
            }) => {
                add_attempt_usage(&mut abandoned_usage, &usage);
...
/// open a fresh attempt — bounded per failure class:
/// [`STREAM_MAX_RETRIES`] for server errors, the deeper
/// [`RATE_LIMIT_MAX_RETRIES`] for rate limits.
async fn back_off_transient_stream_error(
    ...
    // the patient schedule; every other recoverable transient keeps
    // the fail-fast server-error schedule.
    let is_rate_limited = matches!(kind, StreamErrorKind::RateLimited(_));
    let (counter, max_retries, backoff) = if is_rate_limited {
        (
            &mut retry.rate_limit,
            RATE_LIMIT_MAX_RETRIES,
            rate_limit_backoff_delay as fn(u32) -> Duration,
        )
    } else {
        (
            &mut retry.transient,
            STREAM_MAX_RETRIES,
            stream_backoff_delay as fn(u32) -> Duration,
        )
    };
    *counter = counter.saturating_add(1);
    let class_retries = *counter;
    ...
    let delay = kind
        .retry_after()
        .map_or_else(|| backoff(class_retries), clamp_retry_after);
    let delay_ms = u64::try_from(delay.as_millis()).unwrap_or(u64::MAX);
    log::warn!(
        "LLM stream {kind:?} on retry {event} (class {class_retries}/{max_retries}) \
    ...
    // sleeping out the full (up to 120s on the rate-limit schedule) delay
    // on an already-cancelled turn. The failed attempt row was already
    // settled by the failure path, so the cancel close inside the race is
    // a no-op here.
    race_wait_against_cancel(
        deps,
        current_attempt,
...
/// crates for a tiny pure helper.  Fail-fast schedule for server
/// errors (and the connectivity probe/stall paths).
fn stream_backoff_delay(attempt: u32) -> Duration {
    backoff_delay(attempt, STREAM_BASE_DELAY_MS, STREAM_MAX_DELAY_MS)
}

/// Patient schedule for [`StreamErrorKind::RateLimited`] failures —
/// same exponential-with-jitter shape as [`stream_backoff_delay`],
/// tuned to outlast provider overload windows
/// ([`RATE_LIMIT_BASE_DELAY_MS`]/[`RATE_LIMIT_MAX_DELAY_MS`]).
fn rate_limit_backoff_delay(attempt: u32) -> Duration {
    backoff_delay(attempt, RATE_LIMIT_BASE_DELAY_MS, RATE_LIMIT_MAX_DELAY_MS)
}
...
/// Context [`poll_stream_item`] needs to close the attempt when the poll races
...
/// Poll the next stream item, racing it against cancellation (stop consuming a
/// billed stream promptly on cancel) and the stall budget (retry a silent
/// half-open connection instead of hanging until a watchdog kills the task).
///
/// `Ok(Some(item))` is the next stream result; `Ok(None)` means the stream
/// ended cleanly; `Err(..)` means the attempt was already closed and the turn
/// must return that error.
async fn poll_stream_item(
    stream: &mut StreamBox<'_>,
...
async fn call_llm_once_inner(
    ...
    // `StreamBox` is `Pin<Box<…>>` and therefore `Unpin`, so it is polled
    ...
    match delta {
        ...
        StreamDelta::Error { message, kind } => {
            ...
            let error = if kind.is_recoverable() {
                // The accumulated usage is filled in by `call_llm_once_inner`,
                // which owns the accumulator, via `StreamAttemptError::with_usage`.
                StreamAttemptError::Recoverable {
                    kind,
                    message,
                    usage: ZERO_ATTEMPT_USAGE,
                }
            } else {
...
async fn close_attempt_with(
    ...
    let params = CloseAttemptParams {
        ...
        stop_reason: None,
        outcome,
        // A failed/cancelled attempt still bills the tokens it streamed before
        // dying, so its audit row records them rather than a misleading zero.
        input_tokens: usage.input_tokens,
        output_tokens: usage.output_tokens,
        cached_input_tokens: usage.cached_input_tokens,
        cache_creation_input_tokens: usage.cache_creation_input_tokens,
...
/// `usage` is the *attempt's own* token spend, which may differ from
/// `response.usage` when the latter carries the whole turn's total (successful
/// attempt + earlier failed attempts). The audit row bills only this attempt.
...
fn preflight_tool_inputs(
    response: &mut llm::ChatResponse,
    sanitizer: Option<&ToolInputSanitizer>,
    context: &ToolInputPreflightContext<'_>,
) -> Result<Vec<PreflightCompletion>> {
    ...
    for block in &mut response.content {
        if let llm::ContentBlock::ToolUse { name, input, .. } = block {
            let outcome = sanitizer(context, name, input).with_context(|| {
                format!("pre-suspension input preflight failed for tool {name}")
            })?;
            if let ToolInputPreflightOutcome::Complete(result) = outcome {
                completions.push(PreflightCompletion {
                    pending_index,
                    result,
                });
            }
...
fn attach_precompleted_results(
    specs: &mut [ChildSpawnSpec],
    completions: &[PreflightCompletion],
) -> Result<()> {
    for completion in completions {
        let spec = specs
            .get_mut(completion.pending_index)
            .context("preflight completion has no matching child spec")?;
        spec.precompleted_result = Some(
            serde_json::to_value(&completion.result)
                .context("serialize preflight tool result for atomic spawn")?,
        );
    }
    Ok(())
}

fn preflight_end_events(
    preflight_calls: &[(usize, PendingToolCallInfo, ToolResult)],
) -> Vec<AgentEvent> {
    preflight_calls
        .iter()
        .map(|(_, call, result)| {
            AgentEvent::tool_call_end(&call.id, &call.name, &call.display_name, result.clone())
        })
        .collect()
}

fn complete_preflight_children(
    deps: &RootTurnDeps<'_>,
    parent_task: AgentTask,
    child_tasks: Vec<AgentTask>,
    preflight_calls: Vec<(usize, PendingToolCallInfo, ToolResult)>,
) -> Result<(AgentTask, Vec<AgentTask>)> {
    let had_preflight = !preflight_calls.is_empty();
    for (pending_index, _call, result) in preflight_calls {
        let child = child_tasks
            .get(pending_index)
            .context("preflight completion has no matching child task")?;
        ensure!(
            child.status == TaskStatus::Completed,
            "preflight child entered store as {:?}, expected Completed",
            child.status
        );
        let expected =
            serde_json::to_value(&result).context("serialize expected preflight tool result")?;
        ensure!(
            child.result_payload.as_ref() == Some(&expected),
            "preflight child durable result does not match its completion"
        );
    }
    if had_preflight {
        // The spawn primitive publishes the precompleted boundary under its
        // own task/event transaction before returning this parent.
...
async fn suspend_at_tool_boundary(
    ...
    let preflight_context = ToolInputPreflightContext {
        thread_id: &inputs.bootstrap.thread_id,
        caller_metadata: inputs.bootstrap.task.caller_metadata.as_ref(),
    };
    let preflight_completions = preflight_tool_inputs(
        &mut response,
        inputs.definition().tool_input_sanitizer(),
        &preflight_context,
    )?;
...
async fn classify_batch_for_inputs(
    ...
    for (pending_index, _, _) in preflight_calls {
        let decision = decisions
            .get_mut(*pending_index)
            .context("preflight completion index is outside routing decisions")?;
        *decision = super::subagent_spawn_selector::SubagentSpawnDecision::SpawnAsTool;
    }
...
async fn spawn_mixed_batch_children(
    ...
    payload: SuspensionPayload,
    delivered_injection_ids: Vec<AgentTaskId>,
    boundary_events: Vec<AgentEvent>,
    now: OffsetDateTime,
...
struct BatchRoutingApplication {
...
async fn apply_reattached_batch_routing(
    ...
    let (subagents, tool_children) = match routing {
        super::subagent_spawn_selector::BatchRouting::AllTools => {
            let tools = specs
                .into_iter()
                .enumerate()
                .map(|(index, spec)| {
                    Ok(crate::journal::store::ToolChildSpawn {
                        spawn_index: u32::try_from(index)
                            .context("tool spawn_index exceeds u32")?,
                        spec,
                    })
                })
        ...
        super::subagent_spawn_selector::BatchRouting::SingleSubagent { spawn_index, plan } => (
            vec![reattached_subagent_entry(spawn_index, &plan)?],
            Vec::new(),
        ),
        super::subagent_spawn_selector::BatchRouting::MultiSubagent { plans } => (
            plans
                .into_iter()
                .map(|(index, plan)| reattached_subagent_entry(index, &plan))
                .collect::<Result<Vec<_>>>()?,
            Vec::new(),
        ),
        super::subagent_spawn_selector::BatchRouting::Mixed {
            ...
            let subagents = plans
                ...
                .map(|(index, plan)| reattached_subagent_entry(index, &plan))
                .collect::<Result<Vec<_>>>()?;
            let tool_children = tool_indices
                .into_iter()
                .map(|index| {
                    let spec = specs
                        .get(index)
                        .cloned()
                        .with_context(|| format!("tool spawn_index {index} has no spawn spec"))?;
                    Ok(crate::journal::store::ToolChildSpawn {
                        spawn_index: u32::try_from(index)
                            .context("tool spawn_index exceeds u32")?,
                        spec,
    ...
    let spawned = Box::pin(super::subagent::spawn_mixed_children_invocations(
        &inputs.bootstrap.task_id,
        &inputs.bootstrap.worker_id,
        &inputs.bootstrap.lease_id,
        super::subagent::MixedChildrenRequest {
    ...
    .await
    .context("spawn attached batch children")?;
    let mut children: Vec<AgentTask> = spawned
        .invocations
        .into_iter()
        .map(|invocation| invocation.invocation_task)
        .collect();
...
fn reattached_subagent_entry(
    spawn_index: usize,
    plan: &super::subagent_spawn_selector::SubagentSpawnPlan,
) -> Result<super::subagent::SubagentBatchEntry> {
    Ok(super::subagent::SubagentBatchEntry {
        child_thread_id: plan.child_thread_id.clone(),
        spec: plan.spec.clone(),
        child_root_input: plan.child_root_input.clone(),
        spawn_index: u32::try_from(spawn_index).context("subagent spawn_index exceeds u32")?,
        child_caller_metadata: plan.child_caller_metadata.clone(),
    })
}

fn invocation_tasks(
    invocations: Vec<super::subagent::SpawnedSubagentInvocation>,
) -> Vec<AgentTask> {
    invocations
        .into_iter()
        .map(|invocation| invocation.invocation_task)
...
/// Per-block ids plus the successful attempt's own usage, bundled so the
/// resume-commit functions stay within the argument budget.
struct ResumeCloseContext<'a> {
    content_ids: &'a ContentIds,
...
/// completed child task, followed by the results' native binary
/// attachments (images/documents).
///
/// Two passes, matching the in-process loop's `append_tool_results`:
/// Anthropic requires EVERY `tool_result` block to precede any other
/// content in the user message, so attachments are appended after the
/// full result run, in result order. Dropping the attachments instead
/// silences every multimodal tool result on the daemon path.
/// Artifact-backed sources (`artifact://…` data) ride along untouched;
/// the pre-call hydration seam restores their bytes for the provider
/// request.
...
pub async fn resume_from_children(
    ...
    let (continuation, suspended_messages, child_ids, child_join_policy) = match &parent.state {
        TaskState::ReadyToResume {
            continuation,
            suspended_messages,
            child_ids,
    ...
    let interim = build_steering_interim(&continuation, &child_ids, deps.task_store)
        .await
        .context("build child resume fan-in")?;
    if !interim.surviving.is_empty() {
        return Box::pin(run_steering_exchange(
            inputs,
            SteeringExchange {
                continuation,
                suspended_messages,
...
struct SteeringExchange {
    ...
    /// Join policy retained when survivors are reattached.
...
/// Build the assistant message re-parked into history: the model's
/// response followed by synthetic re-attach tool uses.
///
/// When the stop reason refused dispatch (`keep_response_tool_use ==
/// false`) the response's own `ToolUse` blocks are dropped: no child owns
/// them, so persisting them would leave ownerless tool-use ids in the
/// replay history.
fn build_steering_assistant_message(
    response: &llm::ChatResponse,
    reattach_blocks: Vec<llm::ContentBlock>,
    keep_response_tool_use: bool,
) -> llm::Message {
    let mut blocks: Vec<llm::ContentBlock> = if keep_response_tool_use {
        response.content.clone()
    } else {
        response
            .content
            .iter()
            .filter(|block| !matches!(block, llm::ContentBlock::ToolUse { .. }))
            .cloned()
            .collect()
    };
...
fn build_steering_continuation(
    ...
    let resolved_tools = inputs
        ...
        .definition
        .resolve_tools(inputs.bootstrap.task.caller_metadata.as_ref());
    let mut pending_tool_calls = extract_pending_tool_calls(response, &resolved_tools);
    pending_tool_calls.extend(reattach_pending);
...
pub async fn resume_for_steering(
    ...
    let (continuation, suspended_messages, child_ids, steering, child_join_policy) = match &parent
        .state
    {
        TaskState::ReadyToResume {
            continuation,
...
struct PreparedSteeringRepark {
    routing: Option<super::subagent_spawn_selector::BatchRouting>,
    payload: SuspensionPayload,
    specs: Vec<ChildSpawnSpec>,
    reattach: Vec<ReattachedChild>,
    suspended_messages: Vec<llm::Message>,
...
async fn prepare_steering_repark(
    ...
    let preflight_completions = if dispatch_provider_tools {
        preflight_tool_inputs(
            &mut response,
            inputs.definition().tool_input_sanitizer(),
            &preflight_context,
        )?
    ...
    let payload = SuspensionPayload {
        continuation,
        suspended_messages: suspended.clone(),
        child_join_policy,
    };
    Ok(PreparedSteeringRepark {
        routing,
        payload,
        specs,
        reattach,
        suspended_messages: suspended,
        events,
        preflight_calls,
    })
}
...
async fn repark_after_steering_exchange(
    ...
    let PreparedSteeringRepark {
        ...
        events,
        preflight_calls,
    } = prepare_steering_repark(inputs, deps, repark, now).await?;
    let mut boundary_events = if preflight_calls.is_empty() {
        Vec::new()
    } else {
        events.clone()
    ...
    boundary_events.extend(preflight_end_events(&preflight_calls));
    let thread_id = &inputs.bootstrap.thread_id;
    let (parent_task, child_tasks, atomic_committed) = if let Some(routing) = routing {
        apply_batch_routing(
            inputs,
            deps,
            BatchRoutingApplication {
                ...
                specs,
                reattach,
                tool_children_context: "spawn tools while re-attaching children",
                wake_workers: preflight_calls.is_empty(),
            },
            now,
        )
        ...
        let child_ids = reattach.into_iter().map(|child| child.child_id).collect();
        let parent = deps
            .task_store
            ...
            .await
            .context("re-park parent after partial child resume")?;
        (parent, Vec::new(), Vec::new())
    };
    ...
    let committed_events = if preflight_calls.is_empty() {
        if events.is_empty() {
            ...
            deps.event_repo
                ...
                .await
                .context("commit child-resume suspension events")?
        }
    } else {
        atomic_committed
    };
...
mod tests {
    ...
    fn tool_response(input: serde_json::Value) -> llm::ChatResponse {
        llm::ChatResponse {
            id: "msg_tool".to_owned(),
            content: vec![llm::ContentBlock::ToolUse {
                id: "tool_1".to_owned(),
    ...
    #[test]
    fn tool_input_sanitizer_mutates_before_continuation_construction() -> Result<()> {
        let mut response = tool_response(serde_json::json!({
            "action": "open",
            "url": "https://example.com/?signed=secret"
        }));
        let thread_id = ThreadId("thread-a".to_owned());
        let context = ToolInputPreflightContext {
            thread_id: &thread_id,
            caller_metadata: None,
        };
        let expected_thread_id = thread_id.clone();
        let sanitizer = move |context: &ToolInputPreflightContext<'_>,
                              name: &str,
                              input: &mut serde_json::Value|
              -> Result<ToolInputPreflightOutcome> {
            assert_eq!(context.thread_id, &expected_thread_id);
            assert_eq!(name, "browser");
            input["url"] = serde_json::json!("https://example.com/");
            Ok(ToolInputPreflightOutcome::Allow)
        };

        assert!(preflight_tool_inputs(&mut response, Some(&sanitizer), &context)?.is_empty());
        let llm::ContentBlock::ToolUse { input, .. } = &response.content[0] else {
    ...
    #[test]
    fn preflight_completion_scrubs_input_and_returns_typed_result() -> Result<()> {
        let mut response = tool_response(serde_json::json!({
            "action": "open",
            "url": "https://example.com/?signed=secret"
        }));
        ...
        let sanitizer = |_context: &ToolInputPreflightContext<'_>,
                         _name: &str,
                         input: &mut serde_json::Value|
         -> Result<ToolInputPreflightOutcome> {
            *input = serde_json::json!({"rejected": true});
            Ok(ToolInputPreflightOutcome::Complete(ToolResult {
                success: false,
        ...
        let completions = preflight_tool_inputs(&mut response, Some(&sanitizer), &context)?;
        assert_eq!(completions.len(), 1);
        assert!(!completions[0].result.success);
        let llm::ContentBlock::ToolUse { input, .. } = &response.content[0] else {
            unreachable!("fixture contains a tool use")
        };
        assert_eq!(input, &serde_json::json!({"rejected": true}));
        Ok(())
    }
}
