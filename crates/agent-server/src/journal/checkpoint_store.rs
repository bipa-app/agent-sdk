...
struct Inner {
    ...
    /// by `turn_number`).
    thread_index: HashMap<ThreadId, Vec<CheckpointId>>,
    /// Uniqueness index: `(thread_id, turn_number)` → checkpoint id.
    turn_uniqueness: HashMap<(ThreadId, u32), CheckpointId>,
}
...
impl CheckpointStore for InMemoryCheckpointStore {
    async fn commit_checkpoint(&self, params: NewCheckpointParams) -> Result<Checkpoint> {
        ...
        if inner.turn_uniqueness.contains_key(&key) {
            return Err(anyhow!(
        ...
        inner.turn_uniqueness.insert(key, checkpoint.id.clone());
        inner
            .thread_index
    ...
    async fn get(&self, id: &CheckpointId) -> Result<Option<Checkpoint>> {
        ...
        let result = inner.checkpoints.get(id).cloned();
        drop(inner);
        Ok(result)
    }
    ...
    async fn list_by_thread(&self, thread_id: &ThreadId) -> Result<Vec<Checkpoint>> {
        ...
        let mut checkpoints: Vec<Checkpoint> = ids
            .iter()
            .filter_map(|id| inner.checkpoints.get(id).cloned())
            .collect();
        drop(inner);
