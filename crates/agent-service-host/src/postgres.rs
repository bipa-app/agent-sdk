...
mod tests {
    ...
    fn executable_migration_bundle_contains_all_migrations() -> Result<()> {
        let migrations = &DURABLE_CORE_MIGRATOR.migrations;
            ...
            migrations.len() == 10,
            "expected 10 executable migrations (durable core + event journal + execution intents + tool audit events + outbox kind + task caller metadata + message head drafts + turn attempt otel ids + idempotency + task otel traceparent), got {:?}",
            migrations
                .iter()
                .map(|migration| migration.version)
                .collect::<Vec<_>>(),
        );
        for (idx, expected) in [1_i64, 2, 3, 4, 5, 6, 7, 8, 9, 10].iter().enumerate() {
            ensure!(
