-- SQLite-dialect mirror of postgres/0001_durable_core.sql
-- Dialect changes (Postgres → SQLite):
...
CREATE TABLE agent_sdk_message_heads (
    ...
    CONSTRAINT agent_sdk_message_heads_history_json_check
        CHECK (json_type(history_json) = 'array'),
...
CREATE TABLE agent_sdk_message_commits (
    ...
    head_version_after INTEGER NOT NULL,
    ...
    CONSTRAINT agent_sdk_message_commits_batch_json_check
        CHECK (json_type(batch_json) = 'array')
);
...
CREATE TABLE agent_sdk_turn_checkpoints (
    ...
    agent_state_snapshot TEXT NOT NULL,
    turn_input_tokens INTEGER NOT NULL,
    turn_output_tokens INTEGER NOT NULL,
    created_at TEXT NOT NULL,
    ...
    CONSTRAINT agent_sdk_turn_checkpoints_messages_json_check
        CHECK (json_type(messages_json) = 'array'),
