-- ENG-9279: SQLite cannot alter CHECK constraints in place. Rebuild the
-- task and idempotency tables to admit durable question states and requests.
    ...
    CONSTRAINT agent_sdk_tasks_waiting_state_check
        CHECK (
            ...
            AND (
                ...
                OR (
                    json_extract(state_json, '$.kind') = 'awaiting_question'
                    AND status = 'awaiting_question'
                    AND pending_child_count = 0
                )
                OR (
                    json_extract(state_json, '$.kind') = 'answered_question'
...
ALTER TABLE agent_sdk_idempotency RENAME TO agent_sdk_idempotency_old;
...
SELECT request_id, kind, fingerprint, result_json, created_at
FROM agent_sdk_idempotency_old;
DROP TABLE agent_sdk_idempotency_old;
