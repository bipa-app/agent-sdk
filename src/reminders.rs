...
impl ReminderConfig {
    ...
    #[must_use]
    pub fn with_tool_reminder(mut self, tool_name: impl Into<String>, reminder: ToolReminder) -> Self {
        self.tool_reminders
...
impl ReminderTrigger {
    ...
    pub fn should_trigger(&self, input: &Value, result: &ToolResult) -> bool {
        match self {
            ...
            Self::InputMatches { field, pattern } => {
                input
                    .get(field)
...
pub mod builtin {
    ...
    pub const READ_SECURITY_REMINDER: &str =
    ...
    pub const BASH_VERIFICATION_REMINDER: &str =
        "Verify this command produced the expected output. If the output doesn't match \
         expectations, consider alternative approaches before retrying the same command.";

    /// Reminder shown after successful edit.
    pub const EDIT_VERIFICATION_REMINDER: &str =
        "The edit was applied. Consider reading the file to verify the changes are correct, \
...
mod tests {
    ...
    fn test_tool_reminder_builders() {
        ...
        assert!(matches!(on_error.trigger, ReminderTrigger::ResultContains(_)));
    }
