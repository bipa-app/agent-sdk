...
- 2026-05-30 — **12·A (`ENG-8713`) API ergonomics + façade curation drafted.** Added `AgentLoop::ask`/`send` (build `ToolContext::new(())` + a `CancellationToken` internally, return assembled assistant text); `run()`/`run_with_options()` now return `impl Future<Output = anyhow::Result<AgentRunState>>` (the JoinHandle path stays on `run_abortable`); `builder().build()` defaults to an in-memory event store (no `Arc` ceremony); a new `pub mod prelude` (~12 newcomer names) and `pub mod advanced` housing the server/host contract types (`TurnSummary`/`TurnOutcome`, `AuditProvenance`/`ToolAuditRecord*`, `EventAuthority`/`LocalEventAuthority`, `Continuation*`/`CONTINUATION_VERSION`, `Listen*`/`Erased*`, `HostDependencies`/`ExecutionContextFactory`) — relocated, not removed. New `SimpleTool` trait + `ToolRegistry::register_simple` so a first custom tool needs no `ToolName`. Providers gained `from_env()`/`try_from_env()`/`API_KEY_ENV` and accept `impl Into<String>` keys. Two intended breaking changes (the `run()` return type and the `advanced` relocation); `cargo-semver-checks` was unavailable in the worktree, so they are documented in the PR body for the 12·F semver gate. `agent-server`/`agent-service-host` import the sub-crates directly (not the façade root) and were unaffected.
